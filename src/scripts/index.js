...
( function ( $ ) {
	...
	$.flexslider = function ( el, options ) {
				...
				setupPaging() {
					...
					if ( slider.pagingCount > 1 ) {
						for ( let i = 0; i < slider.pagingCount; i++ ) {
							...
							if (
								undefined === slide.attr( 'data-thumb-alt' )
							) {
		...
		slider.flexAnimate = function (
			...
			if (
				...
				if ( asNav && withSync ) {
					const master = $( slider.vars.asNavFor ).data(
						'flexslider'
					);
			...
			if ( ! fade ) {
				...
				if ( vertical && ! carousel ) {
					...
					slider.newSlides.css( {
						...
						float: slider.vars.rtl ? 'right' : 'left',
						display: 'block',
					} );
					slider.container.css( {
						height: '',
					...
					if ( ! touch ) {
						...
						slider.slides
							.css( {
								...
								display: 'block',
								webkitTransition:
									'opacity ' + slider.vars.fadeFirstSlide
										? 1
		...
		slider.doMath = function () {
			...
			slider.firstRowCount =
				slider.itemsPerRow > slider.count
					? slider.itemsPerRow
					: Math.min( slider.itemsPerRow, slider.count );
	...
	$( window )
		...
		.on( 'load', function () {
			$(
				'.woocommerce-product-gallery__wrapper .flexslider-deferred'
			).each( function () {
				$( this )
					.prop( {
